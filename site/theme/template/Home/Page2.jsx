import React from 'react';
import OverPack from 'rc-scroll-anim/lib/ScrollOverPack';
import QueueAnim from 'rc-queue-anim';
import { Link } from 'bisheng/router';
import { Button } from 'antd';
import { FormattedMessage } from 'react-intl';
import * as utils from '../utils';

function Page2() {
  const isZhCN = utils.isZhCN(location.pathname);
  return (
    <div className="home-page page2">
      <div className="home-page-wrapper">
        <div className="title-line-wrapper page2-line">
          <div className="title-line" />
        </div>
        <h2>
          开始使用 <span>Lor</span>
        </h2>
        <OverPack>
          <QueueAnim key="queue" type="bottom" leaveReverse className="page2-content">
            <p key="p" className="page-content">
              <FormattedMessage id="app.home.letspro" />
            </p>
            <div key="code1" className="home-code">
              <div>
                $ <span>git clone</span> https://github.com/sumory/lor --depth=1
              </div>
              <div>$ cd lor</div>
              <div>$ make install</div>
              <div>$ lord new lor_demo</div>
              <div>$ cd lor_demo && lord start</div>
              <div>
                <span className="home-code-comment">
                  {'#'} <FormattedMessage id="app.home.openbrowser" /> http://localhost:8888
                </span>
              </div>
            </div>
            <p key="p2" className="page-content">
              <FormattedMessage
                id="app.home.needhelp"
                values={{
                  docs: (
                    <Link to={utils.getLocalizedPathname('/docs/getting-started', isZhCN)}>
                      <FormattedMessage id="app.home.docs" />
                    </Link>
                  ),
                  faq: (
                    <Link to={utils.getLocalizedPathname('/docs/issues', isZhCN)}>
                      <FormattedMessage id="app.home.faq" />
                    </Link>
                  ),
                  ask: (
                    <a href="https://github.com/sumory/lor/issues">
                      <FormattedMessage id="app.home.ask" />
                    </a>
                  ),
                }}
              />
            </p>
            <div key="button" style={{ marginTop: 88 }}>
              <a
                href="http://github.com/sumory/lor"
                target="_blank"
                rel="noopener noreferrer"
              >
                <Button type="primary">
                  <FormattedMessage id="app.home.download" /> Lor
                </Button>
              </a>
            </div>
          </QueueAnim>
        </OverPack>
      </div>
    </div>
  );
}

export default Page2;
